...
/* We choose a mutex (not spinlock) because the timer callback can     */
/* sleep (e.g., kmalloc with GFP_KERNEL) and because ioctl handlers   */
/* run in process context — a mutex is the right primitive here.       */
/* ------------------------------------------------------------------ */
